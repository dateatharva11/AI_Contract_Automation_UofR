...
import { useArchitects, useCreateArchitect } from "@/hooks/use-architects";
...
export default function ArchitectsList() {
  ...
  const { mutate: createArchitect, isPending } = useCreateArchitect();
  ...
  const filteredArchitects = architects?.filter(v => 
    ...
    <div className="space-y-6 animate-in fade-in duration-500">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-muted-foreground mt-1">Manage approved university architects.</p>
        </div>
        ...
        {user?.role === 'contract_manager' && ( // Added optional chaining
          <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
            ...
            <DialogContent className="sm:max-w-[500px]">
              <DialogHeader>
                <DialogTitle className="font-display text-2xl">Add New Architect</DialogTitle>
              </DialogHeader>
              <Form {...form}>
                <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4 pt-4">
                  ...
                  <DialogFooter className="pt-4">
                    ...
                    <Button type="submit" disabled={isPending}>{isPending ? "Adding..." : "Add Architect"}</Button>
                  </DialogFooter>
                </form>
              </Form>
            </DialogContent>
